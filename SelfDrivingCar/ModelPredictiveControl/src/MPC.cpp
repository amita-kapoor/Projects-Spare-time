...
// We set desired speed for the cost function (i.e. max speed)
const double ref_v = 120;

// The solver needs all the state and actuator variables in a singular vector
// So we define the start and end of variables, making it easier later.
size_t x_start = 0;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Costs for steering and acceleration 
    for (unsigned int t = 0; t < N-1; t++) {
      fg[0] += delta_cost_weight * CppAD::pow(vars[delta_start + t], 2);
      fg[0] += a_cost_weight * CppAD::pow(vars[a_start + t], 2);
    }
    
    // Costs related to the change in steering and acceleration (it helps to make the ride smooth)
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (unsigned int i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -Max_Limit;
    vars_upperbound[i] =  Max_Limit;
  }
  
    // The upper and lower limits of delta set to lie between -25 to 25
  // degrees (values in radians).
  for (unsigned int i = delta_start; i < a_start; i++ ) {
    vars_lowerbound[i] = -Deg25_Limit*Lf;
    vars_upperbound[i] =  Deg25_Limit*Lf;
  }

  // Acceleration/decceleration upper and lower limits.
  for (unsigned int i = a_start; i < n_vars; i++) {
    vars_lowerbound[i] = -Acc_Limit;
    vars_upperbound[i] =  Acc_Limit;
  }
